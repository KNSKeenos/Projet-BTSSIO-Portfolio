...
def extraire_image(article, nom_site):
    ...
    # 1. Tentative via les balises média de feedparser
    if 'media_content' in article and len(article.media_content) > 0:
        return article.media_content[0].get('url')
        
    # 2. Tentative via les fichiers joints (enclosures)
    if 'enclosures' in article and len(article.enclosures) > 0:
        for enc in article.enclosures:
            if enc.get('type', '').startswith('image/'):
                return enc.get('url')
                
    # 3. Image de secours (Unsplash libre de droits) adaptée au thème si aucune image n'est trouvée
    images_secours = {
        "CERT-FR": "https://images.unsplash.com/photo-1563986768609-322da13575f3?w=400", # Sécurité / Cyber
        "Sécurité Debian": "https://images.unsplash.com/photo-1629654297299-c8506221ca97?w=400", # Linux / Serveur
        "IT-Connect": "https://images.unsplash.com/photo-1558494949-ef010cbdcc31?w=400", # Réseau / Infra
    }
    # Renvoie l'image associée ou une image tech générique
...
def generer_json_veille():
    ...
    for nom_site, url_flux in sources_rss.items():
        ...
        for article in flux.entries[:4]:
            ...
            # Extraction de l'image grâce à notre nouvelle fonction
